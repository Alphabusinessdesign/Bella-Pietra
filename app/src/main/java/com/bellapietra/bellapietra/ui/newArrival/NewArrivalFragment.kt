...
class NewArrivalFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        newArrivalAdapter = NewArrivalAdapter(NewArrivalClickListener {
            newArrivalViewModel.onNewArrivalItemClick(it)
        })
        ...
        //Observe when to navigate to the ItemDetails Fragment
        newArrivalViewModel.navigateToItemDetailsFrag.observe(viewLifecycleOwner, Observer {
            it?.let {
                findNavController().navigate(
                    NewArrivalFragmentDirections.actionNavigationDashboardToItemDetailsFragment(
                        getString(R.string.newest_arrival),
                        it
                    )
                )
                newArrivalViewModel.doneNavigating()
            }
        })
        return binding.root
    }
